...
## Git authentication and attribution

All git writes use short-lived GitHub App **installation tokens** — no user tokens are ever stored. Connecting GitHub captures only the user's GitHub identity (`github_user_identities`: login/name/email, one row per WorkOS user), which sets the git author (`login` + `login@users.noreply.github.com`) so commits are attributed to the user on GitHub even though the app pushes. Pushes and PRs act as the app bot.

On every project open a freshly minted installation token is injected into the user's sandbox as working credentials — `~/.git-credentials` + `credential.helper store` and `~/.config/gh/hosts.yml` — so the agent inside the VM can `git push` / `gh pr create` directly. Installation tokens expire after ~1h, so long-idle sandboxes need a project re-open before in-VM pushes work again; server-mediated commit/push/PR routes always mint their own token and are unaffected.
